...
public:
	...
	explicit manifest(const std::string &xml_path)
	{
		...
		auto status = decode_manifest(xml_path);
		if (status == false)
		{
		...
		auto is_debug_string = std::string {xml_doc.child("manifest").child("application").attribute("android::debuggable").as_string()};
		...
		// application class
		application_class_name_ = xml_doc.child("manifest").child("application").attribute("android:name").as_string();
